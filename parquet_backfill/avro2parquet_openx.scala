...
def convert(fs:FileSystem,file:Path):Unit={
  ...
  val data = tokens(tokens.length-1).replace(".avro","")
  ...
  if(table.equals("ox_data_summary_ad_hourly")){
    println("Skipping "+table)
    return
  }
  val newbasefile:String = "parquet_backfill/openx/"+table+"/"+year+"/"+month+"/"+day
